...
const PRESET_COLORS = [
  '#CC0000', '#0044CC', '#00AA44', '#CCCC00',
  '#CC6600', '#880088', '#CCCCCC', '#111111',
];
...
export default function GaragePage() {
  ...
  useEffect(() => {
    ...
    api.get<GarageData>('/garage').then((data) => {
      setGarage(data);
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg)' }}>
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b"
        ...
        <div>
          <span className="font-bold" style={{ color: 'var(--accent)' }}>{garage.apodo}</span>
          <span className="text-xs ml-2" style={{ color: 'var(--text-muted)' }}>
            {COCHERA_NOMBRES[garage.cochera_nivel]}
          </span>
        </div>
        <div className="flex items-center gap-4 text-sm">
          <span>💵 <strong>${garage.pesos.toLocaleString('es-AR')}</strong></span>
          <span>⭐ <strong>{garage.reputacion}</strong></span>
          <button onClick={handleLogout} className="text-xs" style={{ color: 'var(--text-muted)' }}>
            Salir
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 flex-col md:flex-row" style={{ minHeight: 0 }}>
        ...
        <div className="flex-1 relative" style={{ minHeight: '400px' }}>
          {activeCar && (
            <PhaserGarage carSlug={activeCar.slug} colorHex={selectedColor} />
          )}
        </div>

        {/* Panel lateral */}
        <aside className="w-full md:w-64 p-4 flex flex-col gap-5 border-t md:border-t-0 md:border-l"
               style={{ borderColor: 'var(--border)', background: 'var(--surface)' }}>
